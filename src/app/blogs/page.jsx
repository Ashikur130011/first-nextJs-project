        ...
        <div>
            {
                ...
                key={blog.id} 
                blog={blog}
                className='border-2 border-orange-800 text-center bg-orange-200 m-4 p-7 space-y-3'
                >
                
                    <h1>Name: {blog.name}</h1>
                    <p>{blog.details}</p>
                    <Link href={`blogs/${blog.id}`}><button className='bg-white p-2 '>See Details</button></Link>
                </div>)
            }
        </div>
    );
